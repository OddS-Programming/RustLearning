...
// Если нет никаких ограничений, то компилятор будет использовать i32 для целочисленных литералов,
// а f64 — для литералов с плавающей точкой.
...
pub fn literals() {
    ...
    //В предыдущем коде используются некоторые понятия, которых мы ранее не касались.
    // Вот краткое пояснение для нетерпеливых читателей:
    //
    // std::mem::size_of_val является функцией, но вызывается с указанием полного пути.
    // Код можно разделить на логические единицы, называемые модулями.
    // В данном случае функция определена в модуле mem, а модуль mem определён в контейнере std.
